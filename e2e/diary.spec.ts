...
test.describe("日记", () => {
  test("创建日记并在列表中展示", async ({ page }) => {
    ...
    await page.getByRole("link", { name: "我的日记" }).click();
    await page.waitForURL("/diary");
    await expect(page.locator(".ant-typography").filter({ hasText: "我的日记" })).toBeVisible();
    ...
    await drawer.getByPlaceholder("一句话标题").fill(title);
    await drawer.getByPlaceholder("写下今天想说的…").fill(content);
    await buttonWithLabel(drawer, "保存").click();
    await expect(drawer).toBeHidden({ timeout: 10_000 });

    await expect(page.getByText(title)).toBeVisible({ timeout: 10_000 });
    await expect(page.locator(".ant-typography-ellipsis").filter({ hasText: content.slice(0, 12) })).toBeVisible();
  });
});
